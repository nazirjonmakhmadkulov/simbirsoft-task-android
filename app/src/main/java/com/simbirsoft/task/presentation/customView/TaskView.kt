...
import com.simbirsoft.task.common.toFormatLocalTime
import com.simbirsoft.task.data.mapping.toViewTaskModel
...
class TaskView(
    ...
    fun setTaskList(list: List<Task>) {
        taskList = list.map { it.toViewTaskModel() }
        sortRects()
        checkIntersect()
        calculateTaskCoords()
        requestLayout()
    ...
    private fun getTimeString(task: TaskViewTaskModel): String {
        return task.timeStart.toFormatLocalTime() + " - " + task.timeEnd.toFormatLocalTime()
    }
